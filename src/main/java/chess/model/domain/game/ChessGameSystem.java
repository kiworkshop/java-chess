...
public class ChessGameSystem {
    ...
    private ChessErrorMessage isValidMove(Position source, Position destination) {
        ...
        if (source.getX() < 0 || source.getX() >= ChessBoard.GRID_SIZE
            ...
            return ChessErrorMessage.ERROR_INPUT_OUT_OF_BOUND;
        }

        // source == destination
        if (source.getX() == destination.getX() && source.getY() == destination.getY()) {
            return ChessErrorMessage.ERROR_WRONG_MOVE;
        }

        // no piece at the source
        if (chessBoard.get(source) == null) {
            return ChessErrorMessage.ERROR_SOURCE_NOT_VALID;
        }

        // check turn
        if (chessBoard.get(source).getColor() != turn) {
            if (turn == PieceColor.WHITE) {
                return ChessErrorMessage.ERROR_WHITE_TURN;
            } else if (turn == PieceColor.BLACK) {
                return ChessErrorMessage.ERROR_BLACK_TURN;
            }
        }
